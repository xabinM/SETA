                        ...
                        sshagent(['ec2-ssh-key']) {
                            sh '''
                                ssh -o StrictHostKeyChecking=no ubuntu@172.26.8.129 "
                                    cd ${DEPLOY_DIR}/Data &&
                                    echo 'Creating .env file for ML API...' &&
                ...
                sshagent(['ec2-ssh-key']) {
                    sh '''
                        ssh -o StrictHostKeyChecking=no ubuntu@172.26.8.129 "
                            ...
                            ss -tlnp | grep ':8081' || echo 'Port 8081 not in use' &&
            ...
            sshagent(['ec2-ssh-key']) {
                sh '''
                    ssh -o StrictHostKeyChecking=no ubuntu@172.26.8.129 "
                        ...
                        cat ${DEPLOY_DIR}/Data/.env || echo 'Data .env file not found' &&
                        cat ${DEPLOY_DIR}/Spark/.env || echo 'Spark .env file not found'
                    "
                '''
            }
        }
