...
public class KafkaAdminConfig {
    @Bean
    public NewTopic topicRaw(@Value("${app.topic-spec.raw.partitions:6}") int p) {
        return new NewTopic("chat.raw.request.v1", p, (short)1);
    }
    @Bean
    public NewTopic topicFilter() { return new NewTopic("chat.filter.result.v1", 6, (short)1); }
    @Bean
    public NewTopic topicPrompt() { return new NewTopic("chat.prompt.built.v1", 3, (short)1); }
    @Bean
    public NewTopic topicAnswer() { return new NewTopic("chat.llm.answer.v1", 6, (short)1); }
    @Bean
